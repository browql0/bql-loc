-- ═══════════════════════════════════════════════════════════════
-- 📅 SYSTÈME DE RÉSERVATIONS - AMÉLIORATIONS (VERSION CORRIGÉE)
-- ═══════════════════════════════════════════════════════════════
-- Date: 2026-01-17
-- Description: Amélioration de la table bookings - CORRIGÉ pour éviter les erreurs
-- ═══════════════════════════════════════════════════════════════
...
-- ⚠️ IMPORTANT: Ne pas créer de nouveaux types ENUM si la colonne existe déjà
-- La table bookings existe déjà avec status en TEXT, on va la garder en TEXT pour l'instant

-- Modifier payment_status si elle n'est pas déjà en TEXT
-- (On garde TEXT au lieu d'ENUM pour éviter les problèmes de migration)
...
RETURNS TABLE (
    ...
    start_date DATE,
    end_date DATE,
    status TEXT,  -- ✅ Gardé en TEXT au lieu d'ENUM
    status_reason TEXT,
    total_price DECIMAL,
    deposit_amount DECIMAL,
    payment_status TEXT,  -- ✅ Gardé en TEXT au lieu d'ENUM
...
BEGIN
    IF TG_OP = 'UPDATE' AND OLD.status != NEW.status THEN
        ...
        -- Créer notification (vérifier que la table existe)
